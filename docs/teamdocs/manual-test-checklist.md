...
| MT-14 | P0 | Điều khiển dữ liệu demo | Manager đặt override S03 (PM2.5/CO₂/noise/nhiệt độ), giữ đủ cửa sổ policy 15 phút, kiểm tra dashboard/Agent/HITL, sau đó tắt override. | Giá trị override được gắn `demo_override`, giữ qua các tick tự động; backend dùng timestamp kích hoạt làm evidence continuity và tạo proposal `pending` khi đủ policy; tắt override thì giá trị mô phỏng tự động quay lại. Audit ghi cả thao tác đặt và gỡ. | Không có phân biệt demo, override mất ngay ở tick tiếp theo, rewrite lịch sử measurement, hoặc không thể quay lại simulator. |
...
| MT-24a | P0 | Alert sau hai mẫu, email mặc định tắt | Chạy spike với chu kỳ 10 giây; kiểm tra sau mẫu vượt ngưỡng thứ nhất và thứ hai khi `ALERT_CONSECUTIVE_MEASUREMENTS=2`, `RESIDENT_ALERT_NOTIFICATIONS_ENABLED=false`. | Mẫu thứ nhất chưa tạo alert; mẫu thứ hai tạo alert UI cho metric tương ứng; không enqueue email cư dân. | Tạo alert từ một mẫu, không tạo sau hai mẫu hợp lệ, hoặc vẫn enqueue email khi flag tắt. |
