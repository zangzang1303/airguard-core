...
def _passes_quality_gate(intent: Intent, data_items: list[Mapping[str, Any]]) -> bool:
    ...
    if intent == Intent.HISTORY:
        return bool(data_items[0].get("items"))
    ...
    if intent == Intent.WEATHER:
        ...
        return any(weather.get(field) is not None for field in ("temperature", "humidity", "wind_speed", "rainfall"))
                ...
                sources.append(
                    {
                        ...
                        "observed_at": item.get("forecast_at"),
                        "source": item.get("source"),
                    }
                )
